...
export function WebsiteStatusDisplay() {
  ...
  useEffect(() => {
    ...
    if (stillChecking && timerRef.current === null) {
      timerRef.current = setInterval(async () => {
        try {
          const { data } = await axios.get(`/api/user/get-websites?userId=${userId}`)
  ...
  const handlePauseMonitor = async (e: React.MouseEvent, websiteId: string | number) => {
    ...
    if (website.isPaused) {
      try {
        setWebsites((prev) => prev.map((site) => (site.id === websiteId ? { ...site, isPaused: false } : site)))
        ...
        setWebsites((prev) => prev.map((site) => (site.id === websiteId ? { ...site, isPaused: true } : site)))
  ...
  const confirmPause = async () => {
    ...
    try {
      ...
      setWebsites((prev) => prev.map((site) => (site.id === websiteToPause.id ? { ...site, isPaused: true } : site)))
      ...
      setWebsites((prev) => prev.map((site) => (site.id === websiteToPause.id ? { ...site, isPaused: false } : site)))
                ...
                >
                  <div className="flex items-center gap-3">
                    <div className="relative flex items-center justify-center w-3 h-3">
                      {website.isPaused ? (
                        ...
                        <>
                          <span className="absolute inline-flex w-full h-full duration-1000 rounded-full opacity-75 animate-ping bg-cyan-400"></span>
                          <span className="relative inline-flex w-2 h-2 rounded-full bg-cyan-400"></span>
                        </>
                      ) : website.isAcknowledged ? (
                        <>
                          <span className="relative inline-flex w-2 h-2 rounded-full bg-yellow-500"></span>
                        </>
                      ) : (
                        <>
                          <span
                            className={`absolute inline-flex w-full h-full duration-1000 rounded-full opacity-75 animate-ping ${
                              website.isUp ? "bg-green-400" : "bg-red-400"
                            }`}
                          ></span>
                          <span
                            className={`relative inline-flex w-2 h-2 rounded-full ${
                              website.isUp ? "bg-green-500" : "bg-red-500"
                            }`}
                          ></span>
                        </>
                      )}
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <Tooltip>
                      ...
                      <TooltipContent side="top">Website is checked every 3 minutes</TooltipContent>
                    </Tooltip>
